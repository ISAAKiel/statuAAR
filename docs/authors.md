# Authors and Citation

## Authors

- **Christoph Rinne**. Author, maintainer.
  [](https://orcid.org/0000-0002-9829-6182)

- **Martin Hinz**. Author. [](https://orcid.org/0000-0002-9904-6548)

- **Nils Mueller-Scheessel**. Author.
  [](https://orcid.org/0000-0001-7992-8722)

- **Hendrik Raese**. Author.\
  0000-0001-9370-1188

## Citation

Source:
[`inst/CITATION`](https://github.com/ISAAKiel/statuAAR/blob/HEAD/inst/CITATION)

Rinne C, Müller-Scheeßel N, Hinz M, Raese H (2026). *statuAAR: Human
stature estimation from bone measures based on multiple formula*. R
package version 0.0.9000, <https://github.com/ISAAKiel/statuAAR>.

    @Manual{statuAAR,
      title = {statuAAR: Human stature estimation from bone measures based on multiple formula},
      author = {Christoph Rinne and Nils Müller-Scheeßel and Martin Hinz and Hendrik Raese},
      note = {R package version 0.0.9000},
      year = {2026},
      url = {https://github.com/ISAAKiel/statuAAR},
    }
