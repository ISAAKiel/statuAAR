...
estimation is given. The authors do not mention, if the mean of all
regressions is used or the first possible stature estimation according
to hierarchical order of correlation. As the authors discuss the
correlation and refere to the work of Trotter & Gleser a hierarchical
order is supposed (s. Siegmund 2010, p. 20). As correlation within male
and females is highly different and provides a sex specific order of
regressions the stature of individuals with indeterminated sex (indet.)
is calculated by the mean of both at each rank mentioning all bone
measures used and the sum of n_measures.
